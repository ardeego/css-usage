...
void function() {
    window.CSSUsage.StyleWalker.recipesToRun.push( function isLogin(/*HTML DOM domNode*/ element, results) {
        ...
        function collectData(tree, results) {
            ...
            jsonRes["url"] = {href: encodeURIComponent(location.href), text: processResults(location.href)};
            jsonRes["referrer"] = encodeURIComponent(document.referrer);
            ...
            // normalize results
            jsonRes["form"].innerTexts = processResults(jsonRes["form"].innerTexts);
            jsonRes["form"].attributes = processResults(jsonRes["form"].attributes);
            jsonRes["form"].input_attributes = processResults(jsonRes["form"].input_attributes);
            jsonRes["a"].innerTexts = processResults(jsonRes["a"].innerTexts);
            jsonRes["a"].attributes = processResults(jsonRes["a"].attributes);
            jsonRes["aggregate"].innerTexts = processResults(jsonRes["aggregate"].innerTexts);
            jsonRes["aggregate"].attributes = processResults(jsonRes["aggregate"].attributes);
        ...
        function processResults(text) {
            var retVal = {login: 0 , signup: 0, purchase: 0, address: 0}
            for (var name in retVal) {
                var  keylist = parsekeys[name]
                for (i = 0; i<keylist.length; i++) {
                    if (normalize(text).indexOf(keylist[i]) !== -1) {
        ...
        function normalize(text) {
            ...
            if (text != "") {
                ...
                retVal = text.toLowerCase().replace(/[^a-z]+/g, '');
            }
            return retVal.trim();
        }
